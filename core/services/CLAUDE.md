...
- **Direct collection links**: `send_digest_email()` and `send_newsletter_email()` link straight to `{frontend_base}/collections/{code}`. We do not redirect through any analytics trampoline — DESIGN.md §9 forbids tracking pixels and we no longer measure email engagement at all.
