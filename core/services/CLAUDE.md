...
`BOOKING_REQUESTED` / `SWAP_REQUESTED` is a **question put to the owner** — accept or reject? — so it must not outlive its answer. `_clear_request_notifications(booking)` deletes it (`user=booking.owner_code`, matched by `payload__booking_code`) and is called from every path that settles a request:
...
- the auto-decline loop inside it — each sibling declined by an ownership-transferring accept had its own request notification, now equally unanswerable.
...
| `send_booking_unavailable_email(booking, thing)` | Owner's accept of a rival SHARE/SWAP request auto-declines this pending sibling | This booking's requester (warm, non-blaming — the thing went to someone else) |
