...
| `allowed_thing_types` | JSONField (list) | No | Per-collection allowlist of Thing types that may be added. Default `[]` (no restriction — anything that the mode/flags otherwise permit). When non-empty, `core/views/things.py::ThingViewSet.create` rejects any thing whose type is not in the list with 400. The Create/Edit Collection forms enforce "pick at least one" for PROPRIETARY collections and force the list to exactly `["GIFT_THING"]` when album mode is on. The Update serializer also runs an orphan check: narrowing the list while existing things would no longer fit returns 400 with a message naming the offending types. v1 only wires the multi-select for PROPRIETARY collections; COMMUNITY collections keep the field empty until the COMMUNITY iteration. |
