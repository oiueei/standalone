...
4. **IDOR protection** - Users can only view profiles of users connected via collections.
5. **DRF permissions** - `IsThingOwner` and `IsCollectionOwner` custom permission classes.
...
- `ThingViewSet` and `CollectionViewSet` use `ModelViewSet` with `DefaultRouter`.
- Custom permissions: `IsThingOwner`, `IsCollectionOwner` in `core/permissions.py`.
- `get_object_or_404` used consistently across all views.
