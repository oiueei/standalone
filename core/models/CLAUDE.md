...
| `age_range` | CharField(8) | No | Optional age bracket (`UP_TO_21` / `22_30` / `31_40` / `41_50` / `51_60` / `61_PLUS` — expanded from four to six brackets in migration 0114, which reset the retired `22_35`/`36_55`/`56_PLUS` values to unanswered). Asked of every user in the profile editor. Per member it's shared only with the owner of a COMMUNITY collection (guests page); in aggregate it appears in any collection owner's stats CSV. Never public. Default empty. |
