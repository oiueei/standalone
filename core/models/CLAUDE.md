...
| `is_minimalist` | BooleanField | No | **Deprecated — the album/minimalist feature was removed from the UI and API.** The column is retained (dormant, always `False`) to avoid a destructive migration; nothing reads or writes it. Drop with a `RemoveField` migration if it is not coming back. |
