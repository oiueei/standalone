...
| `CollectionCreateSerializer` | headline, description, mode, digest_frequency, is_swap, is_share, newsletter_enabled | Input for collection creation. `is_swap` and `is_share` are mutually exclusive and require COMMUNITY mode. `newsletter_enabled` requires `is_share`. |
| `CollectionUpdateSerializer` | headline, description, status, mode, digest_frequency, is_swap, is_share, newsletter_enabled | Input for collection updates. Same validation as create. |
